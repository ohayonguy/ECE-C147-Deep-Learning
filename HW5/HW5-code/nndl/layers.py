...
def affine_backward(dout, cache):
  ...
  dx = np.array(dout).dot(w.T).reshape(x.shape)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x_hat, x, running_mean, running_var, eps, gamma)
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x_hat, x, mean, var, eps, gamma = cache
  N, D = x.shape
  dbeta = np.sum(dout, axis=0)
  dgamma = np.sum(dout * x_hat, axis=0)
  dl_dxhat = dout * gamma
  dl_dvar = (-1 / 2) * np.sum((1 / ((var + eps) ** (3 / 2))) * (x - mean) * dl_dxhat, axis=0)
  dl_dmean = (-1 / (np.sqrt(var + eps))) * np.sum(dl_dxhat, axis=0)
  dx = np.array((1 / np.sqrt(var + eps)) * dl_dxhat + (2 * (x - mean) / N) * dl_dvar + (1 / N) * dl_dmean)
