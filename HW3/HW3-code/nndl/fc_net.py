...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    layer_1_out, layer_1_cache = affine_relu_forward(np.array(X), self.params['W1'], self.params['b1'])
    scores, layer_2_cache = affine_forward(np.array(layer_1_out), self.params['W2'], self.params['b2'])
    ...
    W1_norm_sqrd = (np.linalg.norm(self.params['W1']) ** 2)
    W2_norm_sqrd = (np.linalg.norm(self.params['W2']) ** 2)

    loss, soft_grad = softmax_loss(scores, y)
    loss += 0.5 * self.reg * (W1_norm_sqrd + W2_norm_sqrd)
    layer_2_back_grad, grads['W2'], grads['b2'] = affine_backward(soft_grad, layer_2_cache)
    layer_1_back_grad, grads['W1'], grads['b1'] = affine_relu_backward(layer_2_back_grad, layer_1_cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1, 0, -1):
      layer_back_grad, grads['W'+str(i)], grads['b'+str(i)] = affine_relu_backward(layer_back_grad, caches[i])
